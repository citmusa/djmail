...
class TestEmailSending(TestCase):
    ...
    def test_simple_send_email(self):
        email = EmailMessage('Hello', 'Body goes here', 'from@example.com', ['to1@example.com', 'to2@example.com'])
        email.send()
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(models.Message.objects.count(), 1)

    @override_settings(EMAIL_BACKEND='djmail.backends.async.EmailBackend',
                       DJMAIL_REAL_BACKEND='django.core.mail.backends.locmem.EmailBackend',
                       DJMAIL_SEND_ASYNC=True)
    def test_async_send_email(self):
        email = EmailMessage('Hello', 'Body goes here', 'from@example.com', ['to1@example.com', 'to2@example.com'])
        # If async is activated, send method returns a thread instead of a number of sent messages.
        email.send().result()
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(models.Message.objects.count(), 1)
...
class TestTemplateEmailSending(TestCase):
    ...
    def test_simple_send_email_1(self):
        ...
        SimpleTemplateMail().send('to@example.com', {'name': 'foo'})
    ...
    def test_simple_send_email_with_magic_builder_1(self):
        email = MagicMailBuilder().test_email2('to@example.com', {'name': 'foo'})
        email.send()
    ...
    def test_simple_email_building(self):
        email = make_email('test_email1', to='to@example.com', context={'name': 'foo'})
    ...
    def test_proper_handlign_different_uses_cases(self):
        email1 = make_email('test_email1', to='to@example.com', context={'name': 'foo'})
        email2 = make_email('test_email2', to='to@example.com', context={'name': 'foo'})
        email3 = make_email('test_email3', to='to@example.com', context={'name': 'foo'})
    ...
    def test_simple_send_email_with_magic_builder_1_with_low_priority(self):
        ...
        mails.test_email2('to@example.com', {'name': 'foo'}, priority=10).send()
